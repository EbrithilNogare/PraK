...
class EditScene extends React.Component {
    ...
    componentDidMount() {
        ...
        if (splitedHref[splitedHref.length - 3] !== 'edit') return
        this.getRecord(
        ...
        return (
            <div>
                {this.state.record !== null && (
                    <Switch>
                        <Route path="/prak/edit/metadata/:id">
                            {' '}
                            <Metadata defaults={this.state.record} />{' '}
                        </Route>

                        <Route path="/prak/edit/corporation/:id">
                            {' '}
                            <Corporation defaults={this.state.record} />{' '}
                        </Route>
                        <Route path="/prak/edit/creation/:id">
                            {' '}
                            <Creation defaults={this.state.record} />{' '}
                        </Route>
                        <Route path="/prak/edit/family/:id">
                            {' '}
                            <Family defaults={this.state.record} />{' '}
                        </Route>
                        <Route path="/prak/edit/geographic/:id">
                            {' '}
                            <Geographic defaults={this.state.record} />{' '}
                        </Route>
                        <Route path="/prak/edit/keyword/:id">
                            {' '}
                            <Keyword defaults={this.state.record} />{' '}
                        </Route>
                        <Route path="/prak/edit/person/:id">
                            {' '}
                            <Person defaults={this.state.record} />{' '}
                        </Route>
                        <Route path="/prak/edit/subject/:id">
                            {' '}
                            <Subject defaults={this.state.record} />{' '}
                        </Route>
                        <Route path="/prak/edit">
                            {' '}
                            <Paper>You must choose what to edit</Paper>{' '}
                        </Route>
                    </Switch>
                )}
            </div>
