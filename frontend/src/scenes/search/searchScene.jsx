      ...
      .then((data) => {
        const records = data.map((a) => {
          ...
          if (a.author && a.author.id)
            toReturn.author = a.author.id.name + " " + a.author.id.surname;
          if (
            a.publishing_date &&
            a.publishing_date[0] &&
  ...
  createSearchParams = (records) => {
    ...
    for (let property of properties) {
      ...
      records.forEach((item) => {
        ...
        if (Array.isArray(itemProperty)) itemProperty = itemProperty.join(", ");
  ...
  setPage(increment = 0) {
    this.setState((prevState) => ({
      page: Math.max(
        Math.min(
          Math.ceil(
            this.state.filteredRecords.length / this.state.itemsOnPage - 1
          ),
  ...
  filterRecords = (records, searchParams, dateFilter) => {
    return records.filter((record) => {
      ...
      Object.entries(searchParams).forEach(([key, value]) => {
        ...
        if (Array.isArray(record[key])) recordKey = record[key].join(", ");
    ...
    return (
      <div className={styles.SearchScene}>
        ...
        <div className={styles.phasesBlock}>
          <Paper className={styles.body}>
            ...
            <PersonComboBox
              ...
              onChange={(e) => {
                this.setDescription("author", e.target.value, false, true);
              }}
            />
            <TextField
              label={typeDefinitionFile.properties["name"].label}
              onChange={(e) => {
                this.setDescription("name", e.target.value, true, true);
              }}
              fullWidth
            />
              ...
              .map((value, key) => (
                <Paper
                  ...
                  onClick={() => {
                    ...
                    this.props.history.push(`/prak/show/metadata/${value.id}`);
                  }}
                >
              ...
              ([searchParamsKey, value]) => {
                ...
                return (
                  <div key={searchParamsKey}>
                    <h3>
                      {typeDefinitionFile.properties[searchParamsKey]?.label ??
                        searchParamsKey}
                      <Button
                        onClick={() => {
                          this.setState((prevState) => {
                            Object.entries(
                              prevState.searchParams[searchParamsKey]
                            ).forEach(
                              ([key, value], index) =>
                            ...
                            onClick={() => {
                              this.setState((prevState) => {
                                prevState.searchParams[searchParamsKey][
                                  ...
                                  !prevState.searchParams[searchParamsKey][name]
                                    .checked;
                                return {
                                  ...prevState,
                                  filteredRecords: this.filterRecords(
        ...
        <Paper className={styles.helperBlock}>
          <h3>Nápověda</h3>
          <b>Vyhledání všech záznamů:</b> nechte všechna pole prázdná a klikněte
          na Vyhledat <br /> <br />
          <b>Interaktivní režim:</b> už během zadávání se ukáže 5 nejlepších
          shod
          <br /> <br />
          <b>Zobrazení záznamu:</b> klikněte kamkoliv na příslušný záznam v
          tabulce níže
        </Paper>
