...
class OneUser extends React.Component {
    ...
    diff = () => {
        ...
        if (this.state.secondName !== this.defaultState.secondName)
            d.secondName = this.state.secondName
        if (this.state.email !== this.defaultState.email)
            d.email = this.state.email
        if (this.state.password !== this.defaultState.password)
            d.password = this.state.password

        if (
            this.state.roleRead !== this.defaultState.roleRead ||
    ...
    saveUser = () => {
        ...
        console.info(
            `%cSaving user with id ${this.props.user._id}\n`,
        ...
        if (Object.keys(diff).length === 0 && diff.constructor === Object) {
            ...
            this.props.enqueueSnackbar('Nic nebylo změněno', {
                variant: 'error',
        ...
        fetch(url, {
            ...
            .then((response) => {
                console.info(
                    '%cUser saved succesfuly\n',
                    'background: #222; color: #bada55'
                )
                this.props.enqueueSnackbar('Změny uloženy', {
                    variant: 'success',
                    autoHideDuration: 6000,
                })
                this.setState({ changed: false })
            ...
            .catch((error) => {
                ...
                this.props.enqueueSnackbar('Ukládání neúspěšné', {
                    variant: 'error',
                    autoHideDuration: 6000,
                })
            })
    ...
    removeUser = () => {
        ...
        console.info(
            `%cremove user ${this.props.user._id}`,
            'background: #222; color: #bada55'
        )
        ...
        fetch(url, {
            ...
            .then((response) => {
                console.info(
                    '%cUser removed succesful',
                    'background: #222; color: #bada55'
                )
                this.props.enqueueSnackbar('Uživatel odstraněn', {
                    variant: 'success',
                    autoHideDuration: 6000,
                })
            ...
            .catch((error) => {
                console.info(
                    '%cUser removed unsuccesful\n',
                    'background: #222; color: #bada55',
                    error
                )
                this.props.enqueueSnackbar('Odstranění neúspěšné', {
                    variant: 'error',
                    autoHideDuration: 6000,
                })
            })
    ...
    render() {
        const { t } = this.props
        if (this.state.removed) return <div />
