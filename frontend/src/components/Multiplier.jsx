...
class Multiplier extends React.Component {
    constructor(props) {
        ...
        const propChildrens = Array.isArray(this.props.children)
            ? this.props.children
            : [this.props.children]

        const notEmptyChild = propChildrens.find(
        ...
        if (
            notEmptyChild &&
            notEmptyChild.props &&
            notEmptyChild.props.defaultValue !== undefined
        ) {
            ...
            propChildrens &&
            propChildrens.props &&
            propChildrens.props.defaultValue !== undefined
        ) {
        ...
        this.state = {
            data:
                dataWithDefaultValues.length !== 0
    ...
    handleRemove = (keyToDelete, childs) => {
        const newState = {
            data: this.state.data.filter((chip) => chip.key !== keyToDelete),
        }
                ...
                {this.state.data.map((value, key) => (
                    <Grid container key={value.key} spacing={1}>
                        <Grid item xs>
                            {propChildrens.map((child, childKey) => {
                                ...
                                return React.cloneElement(child, {
                                    ...
                                    key: childKey,
                                    onChange: (e) => {
                                        child.props.onChange(e, value.key)
                                    },
                                    defaultValue: child.props.defaultValue
                                        ? child.props.defaultValue[value.key] ||
                                          undefined
                                        : undefined,
                                })
                            })}
                        ...
                        <Grid item style={{ width: '60px' }}>
                            <Button
                                onClick={() =>
                                    this.handleRemove(value.key, propChildrens)
                                }
                                style={{
                                    height: '100%',
                                    width: '60px',
