...
class Metadata extends IndexParent {
	constructor(props){
		...
		this.formData.documentType = this.props.defaults ? this.props.defaults.documentType || 0 : 0
		...
		return(
			...
			>
				...
				<FoldablePaper className={styles.dataBlock}> <h2>Autor</h2>
					...
					<Multiplier>
						{this.conditionalField("other_authors_person") && <PersonComboBox {...this.createFieldProps("other_authors_person")}/>}
						{this.conditionalField("other_authors_person_role") && <StaticComboBox {...this.createFieldProps("other_authors_person_role")}/>}
					</Multiplier>
					<Multiplier>
				...
				<FoldablePaper className={styles.dataBlock}> <h2>Detaily</h2>
					...
					<Multiplier>{this.conditionalField("edition_number") && <TextField  {...this.createFieldProps("edition_number")}/>}</Multiplier>
				...
				<FoldablePaper className={styles.dataBlock}> <h2>Obsahová charakteristika</h2>
					...
					<Multiplier>{this.conditionalField("topic_place") && <GeographicComboBox {...this.createFieldProps("topic_place")}/>}</Multiplier>
					<Multiplier>{this.conditionalField("topic_event") && <SubjectComboBox {...this.createFieldProps("topic_event")}/>}</Multiplier>
					<Multiplier>{this.conditionalField("topic_creation") && <CreationComboBox {...this.createFieldProps("topic_creation")}/>}</Multiplier>
					<Multiplier>{this.conditionalField("topic_keyword") && <KeywordComboBox {...this.createFieldProps("topic_keyword")}/>}</Multiplier>
					<Multiplier>{this.conditionalField("topic_family") && <FamilyComboBox {...this.createFieldProps("topic_family")}/>}</Multiplier>
					...
					<Multiplier>
						...
						{this.conditionalField("chronological_content_specification_active") && <LabeledCheckbox {...this.createFieldProps("chronological_content_specification_active")}/>}
					</Multiplier>
