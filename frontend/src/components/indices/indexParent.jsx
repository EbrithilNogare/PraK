...
class IndexParent extends React.Component {
    ...
    createFieldProps = (name) => {
        const typeDefinition = this.getTypeDefinition(name)

        if (typeDefinition === undefined)
            throw new Error('createFieldProps: cannot find: [' + name + ']')

        const { helper, schema, fields, ...definition } = typeDefinition

        if (helper) definition.InputProps = this.helperProp(helper)

        if (this.props.defaults) {
            if (Array.isArray(schema)) {
                const mapOfDefaluts = schema.map((value) =>
                    DeepValue(this.props.defaults, value)
                )
        ...
        definition.onChange = (e, multiplierIndex) => {
            this.handleFormChange(e, schema, multiplierIndex)
        }
        return definition
    }
        ...
        for (let element of elements)
            if (
                ...
                (element.getAttribute('aria-invalid') === 'true' ||
                    element.getAttribute('dbnotsynced') === 'true')
            ) {
    ...
    handleSubmit = (event) => {
        ...
        if (errors.length > 0) {
            errors.forEach((value) => {
                console.warn(value)
                this.props.enqueueSnackbar(value, {
                    variant: 'warning',
            ...
            console.error(
                `Cannot send data, because of ${errors.length} errors`
            )
        ...
        console.info(
            `%cSending data as ${method}:\n`,
            'background: #222; color: #bada55',
            data
        )

        fetch(
            `/prak/api/${this.indexURL}${
                this.indexURL === 'metadata' ? '' : 'index'
            }${method === 'PATCH' ? '/' + this.props.defaults._id : ''}`,
            {
                method,
                headers: {
            ...
            .then((response) => {
                ...
                this.props.enqueueSnackbar(
                    `Sending succesful\nID: ${
                        response.id || this.props.defaults._id
                    }`,
                    { variant: 'success', autoHideDuration: 6000 }
                )
                this.props.history.push(
                    `/prak/show/${this.indexURL}/` +
                        (response.id || this.props.defaults._id)
                )
            })
            .catch((error) => {
